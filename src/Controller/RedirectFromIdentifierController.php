...
class RedirectFromIdentifierController extends ControllerBase {
  ...
  public function main($identifier) {
    ...
    $ids = $data_source->getData($identifier);

    // Provided identifier didn't match anything.
    if (count($ids) === 0) {
      throw new NotFoundHttpException();
    }
    // If there's only one member of $ids, redirect the user.
    if (count($ids) === 1) {
      global $base_url;
    ...
    // If there is more than one entity found, render the template with
    // all corresponding URLsallow the user to choose one they want.
    if (count($ids) > 1) {
      return [
        '#ids' => $ids,
